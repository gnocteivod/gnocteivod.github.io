...
// === HYBRID AUTH: Support both Firebase and localStorage login ===
...
function initDashboardAuth() {
    ...
    const wn = document.getElementById('user-welcome-name');
    if (wn) wn.textContent = (localUser.name || 'Bạn') + ' ✨';
    
    // Auth zone display
    const authZone = document.getElementById('auth-zone');
    if (authZone) {
        authZone.innerHTML = `
        <div id="avatar-btn" class="flex items-center gap-3 bg-white/50 backdrop-blur-sm p-1 pr-4 rounded-full border border-pink-100 shadow-sm cursor-pointer" onclick="document.getElementById('dropdown-menu').classList.toggle('active')">
            <img src="${localUser.avatar || 'https://api.dicebear.com/7.x/adventurer/svg?seed=' + (localUser.uid || localUser.name)}" 
            ...
            <div class="flex flex-col">
                ...
                <span class="text-xs text-pink-600 truncate max-w-[100px] font-bold">${localUser.name || 'User'}</span>
            </div>
        </div>`;
    }
...
function loadMyOrders() {
    ...
    if (!container) return;
    if (!orders.length) {
        container.innerHTML = '<div class="text-center py-16 text-gray-400"><i class="fa-solid fa-ticket text-5xl mb-4 opacity-30"></i><p class="font-bold">Bạn chưa có vé nào</p><a href="detail.html" class="mt-4 inline-block bg-pink-500 text-white px-6 py-2 rounded-full font-bold text-sm">Khám phá sự kiện</a></div>';
        return;
    }
    container.innerHTML = orders.map(o => `
        <div class="group bg-white p-6 rounded-[2.5rem] shadow-sm hover:shadow-md transition-all border-2 border-transparent hover:border-pink-200 flex flex-wrap md:flex-nowrap items-center gap-6">
            <div class="w-20 h-20 rounded-3xl overflow-hidden border-2 border-white shadow-md bg-pink-100 flex items-center justify-center text-3xl">🎫</div>
            <div class="flex-1">
                <span class="bg-pink-100 text-pink-500 text-[9px] font-black px-2 py-0.5 rounded-full uppercase">Vé đã mua</span>
                <h4 class="text-lg font-black mt-1 uppercase">${o.event || 'Sự kiện'}</h4>
                <p class="text-xs font-bold text-gray-400 mt-1">Ghế: ${(o.seats||[]).join(', ')||'–'} &nbsp;·&nbsp; ${o.date}</p>
                <p class="text-sm font-black text-pink-500 mt-0.5">${(o.total||0).toLocaleString('vi-VN')}đ</p>
            </div>
            <div class="text-right">
                <p class="text-[10px] font-bold text-gray-400">Mã đơn</p>
                <p class="font-black text-blue-600 font-mono text-sm">#${o.id}</p>
            </div>
        </div>
    `).join('');
}
...
document.addEventListener('DOMContentLoaded', () => {
 ...
 // --- 1. NHẬP LIỆU FIREBASE ---
    ...
    function updateTopBarUI() {
        const userLocal = JSON.parse(localStorage.getItem('currentUser') || localStorage.getItem('userLogin'));
        if (userLocal) {
    ...
    const _fbAuthHandler = (user) => { if (!user) return; // Firebase user, merge with localStorage
    /* onAuthStateChanged wrapped */
        if (user) {
            // Lấy dữ liệu cũ nếu có để không ghi đè mất SĐT/Địa chỉ khi login lại
            let existingData = JSON.parse(localStorage.getItem('currentUser') || localStorage.getItem('userLogin')) || {};
...
function updateUserProfile() {
    ...
    let userLocal = JSON.parse(localStorage.getItem('currentUser') || localStorage.getItem('userLogin')) || {};
...
// Khởi tạo mặc định
renderMyTickets();
// Fallback logout for non-firebase users
window.logoutFallback = function() {
